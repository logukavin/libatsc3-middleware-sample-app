package com.nextgenbroadcast.mobile.middleware.atsc3.ndk;
...
import com.nextgenbroadcast.mobile.middleware.atsc3.ndk.media.ATSC3PlayerFlags;
import com.nextgenbroadcast.mobile.middleware.atsc3.ndk.media.sync.mmt.MfuByteBufferFragment;
import com.nextgenbroadcast.mobile.middleware.atsc3.ndk.media.sync.mmt.MmtPacketIdContext;
import com.nextgenbroadcast.mobile.middleware.atsc3.ndk.media.sync.mmt.MpuMetadata_HEVC_NAL_Payload;
import com.nextgenbroadcast.mobile.middleware.atsc3.ndk.phy.BwPhyStatistics;
import com.nextgenbroadcast.mobile.middleware.atsc3.ndk.phy.RfPhyStatistics;
...
public class atsc3NdkClient {
    ...
    public interface MediaListener {
        void pushMfuByteBufferFragment(MfuByteBufferFragment mfuByteBufferFragment);
    ...
    public atsc3NdkClient(@NonNull File cacheDir, @NonNull ClientListener listener) {
    ...
    int atsc3_onMfuPacket(int packet_id, int mpu_sequence_number, int sample_number, ByteBuffer byteBuffer, int length, long presentationTimeUs, int mfu_fragment_count_expected) {
        ...
        if (mediaListener != null) {
            if (ATSC3PlayerFlags.ATSC3PlayerStartPlayback) {
                ...
                mediaListener.pushMfuByteBufferFragment(mfuByteBufferFragment);
            } else {
                //discard...
            }
        }
    ...
    int atsc3_onInitHEVC_NAL_Packet(int packet_id, int mpu_sequence_number, ByteBuffer byteBuffer, int length) {
       // Log.d("AT3DrvIntf", "onInitHEVC_NAL_Packet, isVideo:"+isVideo+", mpu_sequence_number: "+mpu_sequence_number+", byteBuffer: "+byteBuffer+", length: "+length);
        ...
        if (mediaListener != null) {
            ...
            mediaListener.pushMpuMetadata_HEVC_NAL_Payload(mpuMetadata_HEVC_NAL_Payload);
        }
    ...
    int atsc3_rf_phy_status_callback(int rfstat_lock,
        ...
        if (mediaListener != null) {
            ...
            mediaListener.pushRfPhyStatisticsUpdate(rfPhyStatistics);
        }
    ...
    int atsc3_updateRfBwStats(long total_pkts, long total_bytes, int total_lmts) {
        if (mediaListener != null) {
            mediaListener.pushBwPhyStatistics(new BwPhyStatistics(total_pkts, total_bytes, total_lmts));
        }
        return 0;
    }
