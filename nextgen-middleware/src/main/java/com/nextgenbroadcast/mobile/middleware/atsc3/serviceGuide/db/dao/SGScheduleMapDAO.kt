...
interface SGScheduleMapDAO {
    ...
    @Query("SELECT sch.*, schc.contentId, schp.startTime, schp.endTime, schp.duration, cnt.icon, cntn.name, cntd.description\n" +
            ...
            "LEFT JOIN (SELECT * FROM (SELECT * FROM content_name WHERE language = :lang UNION ALL SELECT * FROM content_name WHERE language NOTNULL) GROUP BY contentId) cntn ON cntn.contentId = schc.contentId\n" +
            "LEFT JOIN (SELECT * FROM (SELECT * FROM content_description WHERE language = :lang UNION ALL SELECT * FROM content_description WHERE language NOTNULL) GROUP BY contentId) cntd ON cntd.contentId = schc.contentId\n" +
