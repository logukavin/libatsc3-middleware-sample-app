...
class PhyChart @JvmOverloads constructor(
    ...
    private val phyGraphSeries: LineGraphSeries<DataPoint> = LineGraphSeries()
    ...
    override fun onFinishInflate() {
        ...
        with(phyChart) {
            title = resources.getString(R.string.phy_snr)
            titleColor = ContextCompat.getColor(context, R.color.white)
            gridLabelRenderer.gridColor = ContextCompat.getColor(context, R.color.chart_grid_color)
            gridLabelRenderer.numVerticalLabels = 4
            addSeries(phyGraphSeries)
        }
    }
    ...
    companion object {
        ...
        private val TIME_PERIOD_FOR_ACTUAL_SNR_MILLISECOND = TimeUnit.MINUTES.toMillis(5)
    }
}