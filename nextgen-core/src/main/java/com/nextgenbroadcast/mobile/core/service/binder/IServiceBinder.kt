...
interface IServiceBinder {
    ...
    companion object {
        const val LIVEDATA_SUBSCRIBE_ALL = 1
        const val LIVEDATA_UNSUBSCRIBE_ALL = 2

        const val LIVEDATA_RECEIVER_STATE = 3
        const val LIVEDATA_SERVICE_LIST = 4
        const val LIVEDATA_SERVICE_SELECTED = 5
        const val LIVEDATA_APPDATA = 6
        const val LIVEDATA_RMP_LAYOUT_PARAMS = 7
        const val LIVEDATA_RMP_MEDIA_URI = 8

        const val ACTION_OPEN_ROUTE = 9
        const val ACTION_CLOSE_ROUTE = 10
        const val ACTION_SELECT_SERVICE = 11
        const val ACTION_RMP_LAYOUT_RESET = 12
        const val ACTION_RMP_PLAYBACK_STATE_CHANGED = 13
        const val ACTION_RMP_PLAYBACK_RATE_CHANGED = 14
        const val ACTION_RMP_MEDIA_TIME_CHANGED = 15

        const val CALLBACK_ADD_PLAYER_STATE_CHANGE = 16
        const val CALLBACK_REMOVE_PLAYER_STATE_CHANGE = 17

        const val ACTION_PLAYER_STATE_CHANGE_PAUSE = 18
        const val ACTION_PLAYER_STATE_CHANGE_RESUME = 19

        const val ACTION_NEED_URI_PERMISSION = 20
